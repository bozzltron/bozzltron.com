...
**Social / Open Graph:** Post **`hero`** (+ dimensions, **`hero_alt`**) drives **`og:image`**, **`twitter:image`**, and related tags via **`layout/_partial/head.ejs`** and **`scripts/open-graph-override.js`**. See the site root **`README.md`** Editorial section for author-facing checklists.
